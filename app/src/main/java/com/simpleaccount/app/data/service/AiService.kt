...
class AiService @Inject constructor() {
    ...
    suspend fun chatWithImage(
        ...
        if (resp.error != null && isMaxTokensIllegal(resp.error)) {
    ...
    private fun applyThinking(body: JSONObject, model: String, level: String) {
        ...
        val glm = m.contains("glm") || m.contains("chatglm") || m.contains("zhipu")
        val effective = ThinkingPolicy.effectiveLevel(model, level)
        val off = effective == "off"
        body.remove("enable_thinking")
        body.remove("thinking")
        if (glm) {
            // 智谱只认 thinking.type；再塞 enable_thinking 会在已开启时仍报「需开启思考模式」
            body.put("thinking", JSONObject().put("type", if (off) "disabled" else "enabled"))
        } else if (off) {
    ...
    suspend fun chatWithTools(
        ...
        try {
            call.execute().use { resp ->
                if (!resp.isSuccessful) {
                    val err = resp.body?.string().orEmpty()
                    val msg = "HTTP ${resp.code}: ${err.take(200)}"
                    val mapped = if (ThinkingPolicy.isThinkingRequiredError(msg))
                        "该模型把思考当作刚需。已按开启发送；若仍看到本提示，请把思考档调到「低/中/高」并保存后再试。"
                    else msg
                    return@withContextIo ToolChatResult("", error = mapped)
                }
