...
ged++
            ...
            val verified = job.txs.mapNotNull { accountRepository.getById(it.id) }
            val bad = verified.filter { it.category != job.category }
            if (bad.isNotEmpty()) {
                return "写库后核验失败，流水号 ${bad.joinToString(",") { it.id.toString() }} 仍不是「${job.category}」。"
            }
            verified.take(8).forEach {
                lines += "- 流水号 ${it.id} ${it.date} ${it.merchant.ifBlank { it.product }} ¥${MoneyUtil.fenToYuan(it.amount)} 现分类=${it.category}"
            }
        }
        val total = jobs.sumOf { it.txs.size }
        return "【账本已核验】改了 $total 笔（实际变更 $changed 笔）。\n" + lines.joinToString("\n")
    }
