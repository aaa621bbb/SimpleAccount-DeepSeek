...
""".trimIndent()
    ...
    suspend fun run(
        ...
        val requestedThinking = settingsRepository.thinkingLevel()
        val thinkingLevel = com.simpleaccount.app.data.service.ThinkingPolicy.effectiveLevel(
            model,
            if (writeFast && !com.simpleaccount.app.data.service.ThinkingPolicy.requiresThinking(model))
                SettingsRepository.THINKING_OFF
            else requestedThinking,
        )
