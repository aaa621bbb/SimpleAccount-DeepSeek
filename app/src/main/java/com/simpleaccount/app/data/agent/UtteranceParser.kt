...
object UtteranceParser {
    ...
    fun parse(
        ...
        var work = s
        if (amountSpan != null) {
            work = (work.substring(0, amountSpan.first) + " " + work.substring(amountSpan.last + 1)).trim()
        }
    ...
    private fun extractAmount(s: String): Pair<Long?, IntRange?> {
        ...
        Regex("""[¥￥]\s*(\d+(?:\.\d{1,2})?)""").find(s)?.let { m ->
            MoneyUtil.parseToFen(m.groupValues[1])?.let { return Pair<Long?, IntRange?>(it, m.range) }
        }
        // 15元 / 15块钱 / 15.5块
        ...
        Regex("""([零一二两三四五六七八九十百千万\d\.点]+)\s*(?:元|块钱|块)""").find(s)?.let { m ->
            parseChineseMoney(m.groupValues[1])?.let { return Pair<Long?, IntRange?>(it, m.range) }
            MoneyUtil.parseChineseToFen(m.groupValues[0])?.let { return Pair<Long?, IntRange?>(it, m.range) }
        }
        // 三块五（无"元"字兜底）
        Regex("""([零一二两三四五六七八九\d]+)\s*块\s*([零一二三四五六七八九\d])\b""").find(s)?.let { m ->
            ...
            if (yuan != null && jiao != null && yuan in 0..10_000_000) {
                return Pair<Long?, IntRange?>(yuan * 100L + jiao * 10L, m.range)
            }
        }
        ...
        Regex("""(?:记|花了|消费|付款|给了|转了)[^\d]{0,4}(\d+(?:\.\d{1,2})?)""").find(s)?.let { m ->
            ...
            MoneyUtil.parseToFen(m.groupValues[1])?.let { fen ->
                return Pair<Long?, IntRange?>(fen, m.range)
            }
                }
            }
        }
        return Pair<Long?, IntRange?>(null, null)
    }
