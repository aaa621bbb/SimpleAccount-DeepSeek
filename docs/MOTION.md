...
- 空列表走 `EmptyState`。
- **不再**横向「再记一笔」芯片（避免导入后再克隆）。差缺/纠错用静止 SoftCard，无额外动画。
...
- 思考深度用户自选（关/低/中/高）。关：`enable_thinking=false`；开：GLM enabled、Qwen `thinking_budget`、o 系列 `reasoning_effort`。读超时 75s，禁止分钟级空转。
- **写账回合**（记/撤/改分类/无感）强制 `THINKING_OFF`，不改用户设置档。
- 意图门控：闲聊不扫账本、不挂工具。查账才注入快照。LEDGER_WRITE 挂全套写工具。
- 写工具立刻回填并带 `rows_affected`；空工具终答必须 grounding。查询窗最大 500；无 id 改分类上限 800。
- 「记一笔 / 撤回 / 改分类 / 无感」走本地会计秒回（写意图最长 4000 字）。
- 识图 `max_tokens` 上限 1024，避免 GLM HTTP 400 code 1210。相对时间/时段解析成 HH:mm，禁止「时间未知」。
- 流式：贴底跟随；用户上滑后不再 `animateScrollToItem` 拉回首行。推理链 `coalesceReasoning` 连续铺流。

## 账本筛选

月份、分类均为 **多值集合**。维内 OR、维间 AND。空集 = 该维全部。禁止单值互斥。搜索框绑即时 `filter`，debounce 只过滤列表，避免 caret 被受控旧值拽回。
