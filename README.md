...
## 已实现功能
- **记一笔**：金额/收支切换；分类两行横向流（游标跟随）；日期默认滚轮（另有胶片/叠卡，设置分别开，不用日历栅格）；时间默认圆盘表（点按+拖中整分磁吸+回弹，另有弧轨/翻页）；商家/商品并排双栏；保存钮常驻底栏；编辑复用
...
- **本地会计（快路径）**：查实数 / 记一笔 / 撤回一笔秒回；写操作跳过思考深度。分析、建议、怎么办交给模型
