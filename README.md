...
## 已实现功能
- **记一笔**：金额/收支切换；分类两行横向流（游标跟随）；日期 11 套（滚轮/胶片/叠卡/时间轴/算盘珠/地平/扇骨/螺线/三柱/罗盘/账页，默认滚轮，不用日历栅格）；时间 11 套（默认 **机械表盘**，另有弧轨/翻页/双柱/双环/直尺/沙漏/双鼓/日晷/节拍器/积木）；商家/商品并排双栏，已建档商家前缀/包含联想；保存钮常驻底栏；编辑复用
- **账单导入**：微信/支付宝 xlsx/csv/zip；表头语义识别；编码检测(UTF-8/GBK)；金额清洗；自动去重；自动分类（商家记忆→关键词规则→其它）；导入进度与结果页
- **账本**：按日倒序+按月分组；搜索；月份/分类多值交叉筛选（维内 OR、维间 AND）+清除；编辑/删除(二次确认)
- **分类管理**：支出/收入 Tab；新增自定义分类(选图标/色)；**已有分类可二次改图标/颜色**（预置可改外观不可改名）；删除非空自动转"其它"
- **统计**：旧口径（饼/折/柱/星期/时段/日历/商家/对照）+ 十个新视角（频次曲线、结构演进、环比水位、商户集中度、类目弹性、帕累托、热力、日火花、流向、雷达）；设置里可自定义顺序/显隐
- **AI 辅助**：对话窗口；开关/BaseUrl/Key(掩码)/模型名设置；API Key 用 EncryptedSharedPreferences 加密存储；"帮我归类商家"批量归类+历史追改；失败明确提示
- **本地会计（快路径）**：查实数 / 记一笔 / 撤回一笔账单 / 开关无感秒回；写工具执行完立刻回填，deepseek-chat 默认不开思考。分析、建议、怎么办交给模型
- **本月体检**：评分等级 + 总览/结构/节奏/风险/建议（「固定」已下架），数值主视觉，点开看对应流水；订阅不进风险；异常日可追溯
- **首页**：压缩主卡，最近记录占剩余半屏以上；拍账单一键识图 + 上传账单
