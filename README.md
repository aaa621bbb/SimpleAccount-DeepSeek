...
- **体验重构（v2.31.4）**：统计「仅二级」去掉一级汇总防双重归属；端侧模型 meta sidecar + 残留清理跨版本恢复；`suggest_merchant_categories`/`classify_merchants` 批量归类禁推脱；规则词表+最长匹配；首页「约」改页内 BottomSheet 防闪退
...
- **端侧与云端统一协议（v2.31.3）**：仅替换推理后端；诚实边界允许「无法执行」；端侧按档 ≥10 款选型
