...
## 已实现功能
- **记一笔**：金额/收支切换；分类两行横向流（游标跟随）；日期 6 套（滚轮/胶片/叠卡/时间轴/算盘珠/地平，默认滚轮，不用日历栅格）；时间 6 套（默认 **2.18 圆盘**，另有弧轨/翻页/双柱/双环/直尺）；商家/商品并排双栏；保存钮常驻底栏；编辑复用
...
- **统计**：环形饼图 + 12 月曲线 + 12 月柱 + 星期分布 + 时段分布 + 日历；设置里可自定义顺序/显隐；口径互不重复
...
- **本月体检**：评分等级 + 总览/结构/节奏/固定/风险/建议，数值主视觉，点开看对应流水；订阅不进风险；异常日可追溯
