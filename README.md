...
## 已实现功能
- **多账本**：自定义名称，默认「主账本」；流水/统计/AI 严格隔离，不会串味
- **记一笔**：金额键盘 + 分类宫格 + 滚轮日期/时间
- **无感记账**：通知监听 + 前台保活，打开开关会拉起守护通知并引导授权
- **账单导入**：微信/支付宝 xlsx/csv/zip；表头语义识别；编码检测(UTF-8/GBK)；金额清洗；自动去重；自动分类（商家记忆→关键词规则→其它）；导入进度与结果页
- **账本**：全部/支出/收入分段；月份、分类用弹层网格而不是长横条
- **分类管理**：支出/收入各 30+ 互不相同的图标
- **统计**：饼图 + 商家排行 + 工作日/周末日均 + 热力日历 + 月份弹层
...
- **本月体检**：环比、订阅雷达、异常日、工作日/周末日均、夜间消费、按速度预估月底
